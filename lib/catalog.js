...
/**
 * Strip trailing slashes and a trailing version segment (`/v1`, `/v2`, …)
 * from a gateway base URL. The catalog always appends its own full paths
 * (`/v1/models`, `/api/user/models`), so a user-supplied base that already
 * carries `/v1` (the OpenAI-SDK convention, which the chat path also accepts)
 * must not leak through as `/v1/v1/models`. Mirrors the derivation the custom
 * (protocol-URL) template performs in `protocols.js`.
 */
 ...
 * The returned base is version-stripped: the catalog suffixes below always
...
function discoveryBase(connection) {
  ...
  const stripped = stripVersion(base);
...
/** Apply the picker filter (chat-only + exclude patterns, matched case-insensitively). */

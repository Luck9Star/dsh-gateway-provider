...
/** The reasoning family of one model, used to select its wire mapping. */
...
/**
 * OpenAI-route thinking fields for a harness reasoning effort. DeepSeek uses
 * `thinking` + `reasoning_effort`, MiniMax `thinking` adaptive/disabled, and
 * generic reasoning models the OpenAI-style `reasoning_effort` (off = omit).
 */
function thinkingFieldsOpenAI(options, entry, model) {
  const effort = options.reasoningEffort;
  if (effort === undefined) return {};
  switch (reasoningFamilyOf(entry, model)) {
    case "deepseek":
    ...
    default:
      if (effort === "off") return {}; // provider-native default
      return { reasoning_effort: effort }; // low / medium / high
  }
}

/** Anthropic-route thinking fields (Claude and other generic reasoning models). */
function thinkingFieldsAnthropic(options, entry, model) {
  const effort = options.reasoningEffort;
  if (effort === undefined) return {};
  if (reasoningFamilyOf(entry, model) !== "generic") return {}; // deepseek/minimax stay native here
  if (effort === "off") return { thinking: { type: "disabled" } };
  const budgetTokens = effort === "low" ? 1024 : effort === "medium" ? 4096 : 8192;
  return { thinking: { type: "enabled", budget_tokens: budgetTokens } };
}

/** Gemini-route thinking config (generic reasoning models only). */
function thinkingFieldsGemini(options, entry, model) {
  const effort = options.reasoningEffort;
  if (effort === undefined) return {};
  if (reasoningFamilyOf(entry, model) !== "generic") return {};
  if (effort === "off") return {}; // provider-native default
...
function serializeOpenAI(options, connection, entry) {
  ...
  const thinking = thinkingFieldsOpenAI(options, entry, options.model);
  return {
    model: options.model,
    messages,
    stream: true,
...
function serializeAnthropic(options, connection, entry) {
  ...
  return {
    ...
    stream: true,
    ...thinkingFieldsAnthropic(options, entry, options.model),
    ...tools !== undefined && tools.length > 0 ? { tools } : {},
    ...options.temperature !== undefined ? { temperature: options.temperature } : {},
...
function serializeGemini(options, connection, entry) {
  ...
  const generationConfig = {
    ...
    ...thinkingFieldsGemini(options, entry, options.model),
  };
  return {

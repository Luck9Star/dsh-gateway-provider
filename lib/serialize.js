...
/**
 * Resolve the DeepSeek-style thinking/effort pair for a harness reasoning
 * effort. `off` disables thinking; `high`/`max` enable it with the effort.
 */
...
function serializeOpenAI(options, connection, entry) {
  ...
  const thinking = isDeepseekFamily(entry, options.model) ? resolveThinking(options) : {};
  return {
    model: options.model,
    messages,
    stream: true,

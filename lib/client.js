...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var T = ZH ? {
			...
			inherit: "Inherit default", flavorHint: "Label only — protocol dispatch is per-model and automatic",
			...
			labelClearHint: "Empty uses the gateway ID",
		...
		function ensureStyles() {
			...
			tag.textContent = [
				...
				".na_gateway{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);border-radius:12px;padding:12px 14px;display:flex;flex-direction:column;gap:10px}",
				".na_gatewayHead{display:flex;align-items:center;gap:8px;flex-wrap:wrap}",
				".na_gatewayName{font-size:14px;font-weight:600}",
				".na_tag{border:1px solid var(--dsw-alias-border-l2);border-radius:999px;padding:1px 8px;font-size:11px;font-weight:500;line-height:17px;color:var(--dsw-alias-label-tertiary);white-space:nowrap;display:inline-flex;align-items:center;gap:4px}",
				".na_route{font-family:var(--dsw-font-mono,ui-monospace,SFMono-Regular,Menlo,monospace);font-size:11px;color:var(--dsw-alias-label-dimmed)}",
				...
				".na_hint{font-size:11px;color:var(--dsw-alias-label-dimmed);line-height:1.5}",
				...
				".na_count{font-size:12px;color:var(--dsw-alias-label-dimmed);white-space:nowrap}",
				...
				".na_modelMeta{font-size:11px;color:var(--dsw-alias-label-dimmed);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
				...
				".na_empty{border:1px dashed var(--dsw-alias-border-l3);border-radius:10px;padding:16px;text-align:center;color:var(--dsw-alias-label-tertiary);font-size:12px;display:flex;flex-direction:column;gap:8px;align-items:center}",
			].join("\n");
		...
		function ConfigForm(props) {
			...
			if (!isDefault && trimmedURL.length === 0) errors.baseURL = T.urlRequired;
			var dirty = ["baseURL", "label", "apiKeyEnv", "flavor", "catalogMode"].some(function (k) {
			...
			var submit = function () {
				...
				props.onSave({
					...
					apiKeyEnv: !isDefault ? values.apiKeyEnv.trim() : "",
					flavor: values.flavor,
					catalogMode: !isDefault ? values.catalogMode : "",
			...
			return h("form", { className: "na_config", onSubmit: function (e) { e.preventDefault(); submit(); } },
				h("div", { className: "na_configGrid" },
					h(Field, { label: T.baseURL, spanAll: true, error: errors.baseURL, hint: isDefault ? T.baseURLClearHint : null },
					...
					h(Field, { label: T.flavor, hint: T.flavorHint },
						h("select", { className: "na_input", value: values.flavor, onChange: function (e) { set("flavor", e.target.value); } },
							...
							GATEWAY_FLAVORS.map(function (f) { return h("option", { key: f, value: f }, f); }),
						),
					),
		...
		function GatewayCard(props) {
			...
			return h("div", { className: "na_gateway" },
				...
				h("div", { className: "na_meta" },
					...
					h("span", { className: "na_metaUrl" }, gw.baseURL || "—"),
		...
		function AddGatewayForm(props) {
			...
			if (trimmedURL.length > 0 && !validURL(trimmedURL)) errors.baseURL = T.urlInvalid;
			var canSubmit = sid.length > 0 && trimmedURL.length > 0 && validURL(trimmedURL) && !errors.id;

			var close = function () { setOpen(false); setStatus(null); };
			var submit = function () {
				if (!canSubmit) return;
				setStatus(null);
				props.onAdd({
			...
			return h("form", { className: "na_addForm", onSubmit: function (e) { e.preventDefault(); submit(); } },
				...
				h("div", { className: "na_addFormGrid" },
					...
					h(Field, { label: T.baseURL, error: errors.baseURL },
						h("input", { className: "na_input" + (errors.baseURL ? " na_inputInvalid" : ""), type: "text", value: gwURL, spellCheck: false, placeholder: T.urlPlaceholder, onChange: function (e) { setGwURL(e.target.value); } }),
					),
					h(Field, { label: T.flavor, hint: T.flavorHint },
						h("select", { className: "na_input", value: gwFlavor, onChange: function (e) { setGwFlavor(e.target.value); } },
							GATEWAY_FLAVORS.map(function (f) { return h("option", { key: f, value: f }, f); }),
						),
					),
		...
		function GatewayModelsSection(props) {
			...
			var upsertModel = function (arr, entry) {
			...
			/** Merge a freshly discovered list into the stored catalog: the gateway
			  ...
			  * absent from the discovery result are marked custom. */
			var mergeDiscovered = function (current, discovered) {
				...
				var updated = discovered.map(function (d) {
					...
					if (prev !== undefined) {
						// A user-edited name (differing from what discovery reported) wins.
						if (typeof prev.name === "string" && prev.name.length > 0 && prev.name !== (prev._discoveredName || prev.id)) entry.name = prev.name;
			...
			/** Write a model entry (default gateway → root `models`, else gateways[i].models). */
			var writeModels = function (gwIdx, transform) {
				if (gwIdx === -1) {
					return mutate([{ op: "set", path: ["models"], value: transform(snapshot.models || []) }]);
				}
				var nextGateways = gateways.slice();
				nextGateways[gwIdx] = Object.assign({}, gateways[gwIdx], { models: transform(gateways[gwIdx].models || []) });
			...
			var discover = function (gwIdx, baseURL, provider) {
				return api.llm.discoverModels({ settingsNs: NS, provider: provider, baseURL: baseURL, apiKey: undefined }).then(function (res) {
					if (!(res && res.result && res.result.ok)) {
			...
			var fetchModels = function (gwIdx) {
				...
				var gw = gwIdx === -1 ? { baseURL: snapshot.baseURL } : gateways[gwIdx];
				...
				discover(gwIdx, gw.baseURL, provider).then(function (found) {
					...
					var patterns = (snapshot.excludePatterns || []).map(function (p) {
			...
			/**
			 * Save gateway config. Empty string means "clear / inherit": root
			 * fields are unset, per-gateway fields are deleted from the entry.
			 * Returns a Promise<{ok, label}>.
			 */
			var saveGatewayConfig = function (gwIdx, v) {
				var ops = [];
				var writes = [];
				if (gwIdx === -1) {
					if (v.baseURL.length > 0) ops.push({ op: "set", path: ["baseURL"], value: v.baseURL });
					else ops.push({ op: "unset", path: ["baseURL"] });
					if (v.label.length > 0) ops.push({ op: "set", path: ["label"], value: v.label });
					else ops.push({ op: "unset", path: ["label"] });
					ops.push({ op: "set", path: ["flavor"], value: v.flavor });
					writes.push(api.settings.mutate({ ns: NS, ops: ops }));
					if (v.apiKey.length > 0) writes.push(api.credentials.set({ ref: defaultKeyEnv, value: v.apiKey }));
				} else {
					var updated = Object.assign({}, gateways[gwIdx] || {});
					updated.baseURL = v.baseURL;
					if (v.label.length > 0) updated.label = v.label; else delete updated.label;
					if (v.apiKeyEnv.length > 0) updated.apiKeyEnv = v.apiKeyEnv; else delete updated.apiKeyEnv;
					if (v.flavor.length > 0) updated.flavor = v.flavor; else delete updated.flavor;
					if (v.catalogMode.length > 0) updated.catalogMode = v.catalogMode; else delete updated.catalogMode;
					var nextGateways = gateways.slice();
					...
					if (v.apiKey.length > 0) {
						var keyRef = v.apiKeyEnv || gateways[gwIdx].apiKeyEnv || defaultKeyEnv;
						writes.push(api.credentials.set({ ref: keyRef, value: v.apiKey }));
					}
				}
			...
			var addGateway = function (spec) {
				if ((gateways || []).some(function (gw) { return gw.id === spec.id; })) {
				...
				if (spec.apiKey !== undefined && spec.apiKey.length > 0) {
					writes.push(api.credentials.set({ ref: spec.apiKeyEnv || defaultKeyEnv, value: spec.apiKey }));
				}
				return Promise.all(writes).then(function (results) {
					var ok = results.every(function (r) { return r && r.result && r.result.ok; });
					if (ok) load();
			...
			return h("div", { className: "na_section" },
				...
				h(GatewayCard, {
					key: "default",
					gateway: { id: "default", label: snapshot.label || "NewAPI", flavor: snapshot.flavor || "newapi", baseURL: snapshot.baseURL },

	...
	factory: (require) => {
		...
		function ConfigForm(props) {
			...
			var submit = function () {
				...
				var changes = {};
				["baseURL", "label", "apiKeyEnv", "flavor", "catalogMode"].forEach(function (k) {
					var cur = (values[k] || "").trim();
					if (k === "baseURL") cur = cur.replace(/\/+$/, "");
		...
		function AddGatewayForm(props) {
			...
			if (trimmedURL.length === 0) {
				if (urlTouched) errors.baseURL = T.urlRequired;
			...
			return h("form", { className: "na_addForm", onSubmit: function (e) { e.preventDefault(); submit(); } },
				...
				h("div", { className: "na_addFormGrid" },
					...
					h(Field, { label: T.baseURL, error: errors.baseURL },
						h("input", { className: "na_input" + (errors.baseURL ? " na_inputInvalid" : ""), type: "text", value: gwURL, spellCheck: false, placeholder: T.urlPlaceholder, onBlur: function () { setUrlTouched(true); }, onChange: function (e) { setGwURL(e.target.value); } }),
					),
					h(Field, { label: T.flavor, hint: T.flavorHint },
		...
		function GatewayModelsSection(props) {
			...
			var saveGatewayConfig = function (gwIdx, changes) {
				...
				if (gwIdx === -1) {
					...
					var touched = false;
					if (changes.baseURL !== undefined) { updated.baseURL = changes.baseURL; touched = true; }
					if (changes.label !== undefined) {
						if (changes.label.length > 0) updated.label = changes.label; else delete updated.label;
						touched = true;
					}
					if (changes.apiKeyEnv !== undefined) {
						if (changes.apiKeyEnv.length > 0) updated.apiKeyEnv = changes.apiKeyEnv; else delete updated.apiKeyEnv;
						touched = true;
					}
					if (changes.flavor !== undefined) {
						if (changes.flavor.length > 0) updated.flavor = changes.flavor; else delete updated.flavor;
						touched = true;
					}
					if (changes.catalogMode !== undefined) {
						if (changes.catalogMode.length > 0) updated.catalogMode = changes.catalogMode; else delete updated.catalogMode;
						touched = true;
					}
					if (touched) {
						var nextGateways = gws.slice();

...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ModelRow(props) {
			...
			var update = function (key, val) { props.onChange(path, m.id, Object.assign({}, m, Object.defineProperty({}, key, val))); };
		...
		function GatewayModelsSection(props) {
			...
			var load = React.useCallback(function () {
				api.settings.describe({}).then(function (res) {
					var desc = (res.namespaces || []).find(function (n) { return n.ns === NS; });
			...
			var mutate = function (ops) {
				return api.settings.mutate({ ns: NS, ops: ops }).then(function () { load(); });
			};

			var onModelChange = function (path, id, patch) {
				// Read current models array at path, replace/merge the model with id.
				var arr = path.length === 1 ? (snapshot[path[0]] || []) : (((snapshot[path[0]] || {})[path[1]] || []).filter(function (g) { return g.id === path[2]; })[0] || {}).models || [];
				...
				mutate([{ op: "set", path: path.concat(["models"]), value: next }]);
			};

			var fetchModels = function (gwIdx) {
				var key = gwIdx === -1 ? "default" : String(gateways[gwIdx].id);
				setFetching(Object.assign({}, fetching, Object.defineProperty({}, key, true)));
				var gw = gwIdx === -1 ? { baseURL: snapshot.baseURL, apiKeyEnv: snapshot.apiKeyEnv } : gateways[gwIdx];
				api.llm.discoverModels({ ns: NS, provider: gwIdx === -1 ? "newapi" : "gateway:" + gw.id, baseURL: gw.baseURL, apiKey: undefined }).then(function (res) {
					var models = res.models || [];
					var path = gwIdx === -1 ? ["models"] : ["gateways", String(gwIdx), "models"];
					mutate([{ op: "set", path: path, value: models.map(function (m) { return { id: m.id, name: m.name }; }) }]);
				}).catch(function () {}).finally(function () {
					var next = Object.assign({}, fetching); delete next[key]; setFetching(next);
				});
			};
			...
			return h("div", { className: "na_section" },
				...
				gateways.map(function (gw, i) {
					return h(GatewayCard, {
						...
						onDelete: function () { mutate([{ op: "unset", path: ["gateways", String(i)] }]); },
						onModelChange: onModelChange, onModelDelete: function () {},
					});
				}),
			);

...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var T = ZH ? {
			...
			baseURLClearHint: "留空则回退到环境变量 NEWAPI_BASE_URL 或官方云端地址；结尾带不带 /v1 均可",
			...
			baseURLClearHint: "Empty falls back to NEWAPI_BASE_URL or the public cloud default; a trailing /v1 is optional",
		...
		function GatewayModelsSection(props) {
			...
			var load = React.useCallback(function () {
				api.settings.describe().then(function (res) {
					var nsList = res && res.ok ? (res.value.namespaces || []) : [];
					...
					return api.credentials.describe(refs).then(function (cres) {
						var creds = (cres && cres.ok) ? (cres.value || {}) : {};
			...
			var mutate = function (ops) {
				return api.settings.mutate(NS, ops, undefined).then(function (res) {
					if (!(res && res.ok)) {
						console.warn("llm-newapi: settings write failed", res && res.error);
						load();
						return false;
					}
					load();
			...
			var discover = function (gwIdx, baseURL, provider) {
				return api.llm.discoverModels(NS, { provider: provider, baseURL: baseURL || undefined }).then(function (res) {
					if (!(res && res.ok)) {
			...
			var saveGatewayConfig = function (gwIdx, changes) {
				...
				if (gwIdx === -1) {
					...
					if (ops.length > 0) writes.push(api.settings.mutate(NS, ops, undefined));
					if (changes.apiKey !== undefined) writes.push(api.credentials.set(defaultEnv, changes.apiKey));
					...
					if (touched) {
						...
						writes.push(api.settings.mutate(NS, [{ op: "set", path: ["gateways"], value: nextGateways }], undefined));
						syncGateways(nextGateways);
					}
					if (changes.apiKey !== undefined) {
						var keyRef = changes.apiKeyEnv || gw.apiKeyEnv || defaultEnv;
						writes.push(api.credentials.set(keyRef, changes.apiKey));
					}
				}
				return Promise.all(writes).then(function (results) {
					var ok = results.every(function (r) { return r === undefined || r === null || (r && r.ok) !== false; });
			...
			var addGateway = function (spec) {
				...
				syncGateways(nextGateways);
				var writes = [
					api.settings.mutate(NS, [{ op: "set", path: ["gateways"], value: nextGateways }], undefined),
				];
				if (spec.apiKey !== undefined && spec.apiKey.length > 0) {
					var keyEnv = (snapRef.current && snapRef.current.apiKeyEnv) || "NEWAPI_API_KEY";
					writes.push(api.credentials.set(spec.apiKeyEnv || keyEnv, spec.apiKey));
				}
				return Promise.all(writes).then(function (results) {
					var ok = results.every(function (r) { return r === undefined || r === null || (r && r.ok) !== false; });
					load();
		...
		var inject = ["slots", "remote", "remote.settings", "remote.credentials", "remote.llm"];
		...
		function apply(ctx) {
			...
			slots.inject("settings.section", function () {
				slots.register({
					...
					inject: function () {
						// Typed Remote namespaces (dsh ≥ 0.1.2-rc.1): the old
						// `ctx.get("connection").api` RPC facade was removed.
						return { api: {

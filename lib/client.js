...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
				...
				cfgOpen ? h("div", { className: "na_config" },
					...
					h("div", { className: "na_configSave" },
						h("button", { className: "na_btn na_btnPrimary", disabled: cfgSaving, onClick: function () {
							...
							onSaveConfig({ baseURL: cfgBaseURL || undefined, apiKey: cfgKey || undefined }).then(function (result) {
								setCfgSaving(false);
								setCfgStatus(result);
								if (result.ok) { setCfgKey(""); setCfgBaseURL(""); }
							});
						} }, cfgSaving ? "…" : T.saveBtn),
		...
		function GatewayModelsSection(props) {
			...
			/**
			 * Save gateway config: writes baseURL to settings and API key to credentials.
			 * Returns a Promise<{ok, label}> for the caller to surface as a status badge.
			 */
			var saveGatewayConfig = function (gwIdx, changes) {
				var keyRef = gwIdx === -1 ? (snapshot.apiKeyEnv || "NEWAPI_API_KEY") : (gateways[gwIdx].apiKeyEnv || "NEWAPI_API_KEY");
				var ops = [];
				if (changes.baseURL !== undefined && changes.baseURL.length > 0) {
					if (gwIdx === -1) ops.push({ op: "set", path: ["baseURL"], value: changes.baseURL.replace(/\/+$/, "") });
					else {
						var nextGateways = gateways.slice();
						nextGateways[gwIdx] = Object.assign({}, gateways[gwIdx], { baseURL: changes.baseURL.replace(/\/+$/, "") });

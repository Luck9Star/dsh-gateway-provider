 ...
 * model control: context window, output-token cap, reasoning support, tool
 * calling, family, and input modalities. The registry keys are namespaced
 ...
 *
 * @module dsh-newapi-provider/modelsdev
 */
...
export function extractModelsDevParams(entry) {
  ...
  const input = Array.isArray(modalities.input) ? modalities.input.filter((m) => typeof m === "string") : [];
  const output = Array.isArray(modalities.output) ? modalities.output.filter((m) => typeof m === "string") : [];
  return {

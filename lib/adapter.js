...
import { reasoningFamilyOf, serializeRequest } from "./serialize.js";
...
/** DeepSeek/MiniMax-family wire: thinking + reasoning_effort / adaptive-disabled. */
const DEEPSEEK_EFFORTS = [
  { id: OFF_REASONING_EFFORT, name: "Off" },
  { id: HIGH_REASONING_EFFORT, name: "High" },
  { id: MAX_REASONING_EFFORT, name: "Max" },
];
/** Generic reasoning wire: OpenAI-style reasoning_effort low/medium/high. */
...
/**
 * Whether the adapter should advertise selectable reasoning efforts, inferred
 * automatically from the models.dev `reasoning` flag: models that declare
 * reasoning support get a selector, everything else keeps provider-native
 * defaults. Per-family wire mappings are applied at serialization time.
 */
function reasoningEffortsOf(entry) {
  if (entry === undefined) return undefined;
  if (entry.reasoning !== true) return undefined; // false / unknown → no inference
  switch (reasoningFamilyOf(entry)) {
    case "deepseek":
    case "minimax":
      return { efforts: DEEPSEEK_EFFORTS };
    default:
      return { efforts: GENERIC_EFFORTS };
  }
}
...
export class NewapiAdapter extends LlmAdapter {
  ...
  async resolveModel(provider, model, signal) {
    ...
    const reasoning = reasoningEffortsOf(configured);
    return {

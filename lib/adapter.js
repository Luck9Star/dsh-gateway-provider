/**
 * NewapiAdapter: one LlmAdapter instance serving every model advertised by any
 * configured gateway, under its own provider route.
 *
 * Each gateway (newapi / litellm / generic / custom) becomes a provider route
 * (`gateway:<id>`, plus the legacy `newapi` route). The adapter is
 * transport-only: connection facts arrive per-gateway through a thunk resolved
 * once per operation, the bearer token through a per-request resolver, and the
 * per-model catalog (gateway list + models.dev parameters) through a per-base-URL
 * TTL cache. Each request picks the model's wire protocol from its advertised
 * `supported_endpoint_types` (or an explicit override) and assembles the URL
 * from the gateway's protocol-path map.
 *
...
import { endpointUrl, pickEndpoint, priorityOf } from "./gateways.js";
import { providerErrorMessage } from "./wire.js";
...
/**
 * Resolve the per-protocol header set for one request. The Anthropic protocol
 * requires a version header; others carry only auth + content-type.
 */
function protocolHeaders(protocol, apiKey, purpose) {
  return {
    ...
    ...protocol === "openai-response" ? { "content-type": "application/json" } : {},
    ...purpose === "compaction" ? { "x-deepseek-harness-compact": "1" } : {},
  };
}
...
export class NewapiAdapter extends LlmAdapter {
  ...
  async resolveModel(provider, model, signal) {
    ...
    const reasoning = thinkingEffortsOf(configured, findPiModel(model), { extended: connection.extendedReasoningLevels });
    return {
  ...
  async *request(options, signal, connection, apiKey, onComment) {
    ...
    const modelOverride = connection.modelOverrides?.[options.model];
    const priority = modelOverride?.endpointPriority ?? connection.endpointPriority ?? priorityOf(connection.flavor);
    const endpoint = pickEndpoint(entry?.endpointTypes, priority, modelOverride?.protocol);
    if (endpoint === undefined) {
      const advertised = entry?.endpointTypes?.join(", ") ?? "unknown";
      throw new LlmError(
        `model "${options.model}" advertises no supported request protocol (${advertised}); adapter implements openai, openai-response, anthropic, gemini`,
        "UNSUPPORTED_ENDPOINT",
      );
    }
    const url = endpointUrl(connection.baseURL, endpoint, options.model, connection.flavor, connection.protocolPaths);
    const body = JSON.stringify(serializeRequest(endpoint, options, connection, entry));
    const headers = protocolHeaders(endpoint, apiKey, options.purpose);
    let response;
    try {
      response = await fetch(url, { method: "POST", headers, body, signal });
    } catch (error) {
      if (signal.aborted) throw error;
      throw new LlmError(`gateway API request to ${url} failed`, "TRANSPORT", { cause: error });
    }
    if (!response.ok) {
      let message = `gateway API error (HTTP ${response.status})`;
      let providerError;
      try {
    ...
    if (!response.body) throw new LlmError("gateway API returned no response body", "EMPTY_RESPONSE");

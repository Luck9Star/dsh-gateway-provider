...
/** Whether the adapter should advertise selectable reasoning efforts for a model. */
function reasoningEffortsOf(entry) {
  if (entry === undefined) return undefined;
  const id = entry.id.toLowerCase();
  const family = typeof entry.family === "string" ? entry.family.toLowerCase() : "";
  // DeepSeek-family models accept the harness reasoning-effort wire fields
  // (thinking + reasoning_effort); MiniMax models accept thinking adaptive/disabled.
  const controllable = family.startsWith("deepseek") || id.startsWith("deepseek")
    || family.startsWith("minimax") || id.startsWith("minimax");

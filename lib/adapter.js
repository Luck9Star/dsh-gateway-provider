...
function reasoningEffortsOf(entry) {
  // Only DeepSeek-family models accept the harness reasoning-effort wire
  // fields through the gateway; other reasoning models keep native defaults.
  if (entry === undefined) return undefined;
  const family = entry.family;
  const deepseek = (typeof family === "string" && family.toLowerCase().startsWith("deepseek"))
    || entry.id.toLowerCase().startsWith("deepseek");

...
export class NewapiAdapter extends LlmAdapter {
  ...
  async resolveModel(provider, model, signal) {
    ...
    const contextWindow = configured?.contextWindow ?? connection.defaultContextWindow ?? DEFAULT_CONTEXT_WINDOW;
  ...
  async piProviderOf(connection, apiKey) {
    ...
    const models = entries.map((entry) => {
      ...
      return buildModel(entry, connection.providerId, connection.baseURL, priority, connection, override);
    });
    ...
    const custom = Object.entries(connection.modelOverrides ?? {})
      ...
      .map(([id, ov]) => buildModel({ id, name: ov.name ?? id, endpointTypes: undefined }, connection.providerId, connection.baseURL, priority, connection, ov));

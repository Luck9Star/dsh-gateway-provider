 ...
 *
 * The provider carries every discovered model; each model's `api` field
 * dispatches pi-ai to the right protocol implementation. A single `api`
 * factory (`openAICompletionsApi`) is passed to `createProvider` as the
 * default stream implementation, but per-model `api` overrides route each
 * model to its own protocol through pi-ai's `Model.api` dispatch.
 *
